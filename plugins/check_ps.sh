...
# Hacked to allow per-user calculations and accurately calculate
# the totals with bc
# Daniel Wittenberg <dwittenberg2008@gmail.com>

# Update grep command to work better
...
print_help() {
   ...
   echo "     then \"greped\"."
   echo "  -u/--user)"
   echo "     Watch all processes from this user"
   echo "  -w/--warning)"
...
do
   if [ "$user"X == "X" ]; then
       ...
       tmp_output=`ps aux | grep "$process" | grep -v $0 | grep -v grep |tee $tmpfile`
   else
       process=""
       tmp_output=`ps u -U $user | grep -v ^USER | grep -v $0 | grep -v grep |tee $tmpfile`
   fi
   ...
   t_ps_cpu=`cat $tmpfile | awk '{print $3}' | sed ':a;N;$!ba;s/\n/+/g' |bc`
   ps_cpu=`echo "$ps_cpu+$t_ps_cpu" | bc`

   t_ps_mem=`cat $tmpfile | awk '{print $4}' | sed ':a;N;$!ba;s/\n/+/g' |bc`
   ps_mem=`echo "$ps_mem+$t_ps_mem" | bc`

   t_ps_vsz=`cat $tmpfile | awk '{print $5}' | sed ':a;N;$!ba;s/\n/+/g' |bc`
   ps_vsz=`echo "$ps_vsz+$t_ps_vsz" | bc`

   t_ps_rss=`cat $tmpfile | awk '{print $6}' | sed ':a;N;$!ba;s/\n/+/g' |bc`
   ps_rss=`echo "$ps_rss+$t_ps_rss" | bc`
   ...
   t_ps_cputime=`echo "scale=0; (${tmp_ps_cpuhours} * 60) + ${tmp_ps_cpumin}" | bc -l`
   ps_cputime=`echo "$ps_cputime+$t_ps_cputime" | bc`

   rm -f $tmpfile
   counter=$(( $counter + 1 ))
done

   ps_cpu=`echo "$ps_cpu/$iteration" | bc`
   ps_cpu=`echo "$ps_cpu/$processors" | bc`
   ps_mem=`echo "scale=1; $ps_mem/$iteration" | bc`
   if [ "$ps_mem" == "0" ]; then
       ps_mem="0.0"
   fi
   ps_vsz=`echo "scale=1; $ps_vsz/$iteration" | bc`
   ps_rss=`echo "scale=1; $ps_rss/$iteration" | bc`
   ps_cputime=`echo "scale=1; $ps_cputime/$iteration" | bc`

   # Convert to MB
   ps_rss=`echo ${ps_rss}/1024 | bc`
   ps_vsz=`echo ${ps_vsz}/1024 | bc`
...
RAM: ${ps_mem}%, ${ps_rss}MB, ${ps_vsz}MB, Start: ${ps_start}, CPU Time: ${ps_cputime} min"
}

do_perfdata() {
	perfdata="'cpu'=${ps_cpu} 'memory'=${ps_mem} 'cputime'=${ps_cputime} 'rss'=${ps_rss}MB 'vsz'=${ps_vsz}MB "
}
